...
class fixed_ring_test: public CxxTest::TestSuite
{
  ...
  typedef flex::fixed_ring<int, 128> ring_int;
  typedef flex::fixed_ring<object, 128> ring_obj;
...
public:
  ...
  void test_default_fill_constructor(void)
  {
    for (unsigned s = 0; s < SIZE_COUNT; ++s)
    {
      ...
      /*
       * Case2: Verify fill constructor with object elements.
       */
      ring_obj b(SIZES[s]);
  ...
  void test_range_constructor(void)
  {
    for (unsigned s = 0; s < SIZE_COUNT; ++s)
    {
      ...
      TS_ASSERT_EQUALS(c.capacity(), 128);
  ...
  void test_copy_constructor(void)
  {
    for (unsigned s = 0; s < SIZE_COUNT; ++s)
    {
      ...
      /*
       * Case2: Verify copy constructor with object elements.
       */
      ring_obj c(OBJ_DATA, OBJ_DATA + SIZES[s]);
      ring_obj d(c);
      TS_ASSERT_EQUALS(d.size(), c.size());
      TS_ASSERT_LESS_THAN_EQUALS(d.size(), d.capacity());
      TS_ASSERT_EQUALS(d.capacity(), 128);
  ...
  void test_full(void)
  {
     ...
     */
    ring_int b(INT_DATA, INT_DATA + 128);
    TS_ASSERT(b.full());
    b.pop_back();
    TS_ASSERT(!b.full());
    b.push_front(0);
