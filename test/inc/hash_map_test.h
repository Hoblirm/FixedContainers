...
public:
   ...
   bool is_container_valid(hash_map& c)
   {
      ...
      if (false)
      {
         ...
         while (pool_it.get_node() != NULL)
         {
            if (pool_it.get_node()->mValue.second.init == obj::INIT_KEY)
            {
               printf("Error: Expected (pool_it->init != object::INIT_KEY), found (%d == %d)\n", pool_it.get_node()->mValue.second.init,
                       obj::INIT_KEY);
               return false;
            }
            ++pool_it;
