...
namespace flex
{
   ...
   template<typename T, typename Alloc>
   template<typename Compare>
   inline typename list<T, Alloc>::iterator list<T, Alloc>::sort(iterator first, iterator last, size_type n, Compare comp)
   {
      switch (n)
      {
         case 0:
         ...
         case 2:
            ...
            {
               first.mNode->mPrev->mNext = last.mNode;
               last.mNode->mNext->mPrev = first.mNode;

               first.mNode->mNext = last.mNode->mNext;
               last.mNode->mPrev = first.mNode->mPrev;

               first.mNode->mPrev = last.mNode;
               last.mNode->mNext = first.mNode;
               return last;
            }
            return first;
         case 3:
            ...
            if (comp(*mid, *first))
            {
               first.mNode->mPrev->mNext = mid.mNode;
               mid.mNode->mNext->mPrev = first.mNode;

               first.mNode->mNext = mid.mNode->mNext;
               mid.mNode->mPrev = first.mNode->mPrev;

               first.mNode->mPrev = mid.mNode;
               mid.mNode->mNext = first.mNode;
            ...
            {
               //Remove last node
               last.mNode->mPrev->mNext = last.mNode->mNext;
               last.mNode->mNext->mPrev = last.mNode->mPrev;
            ...
            {
               mid.mNode = static_cast<node_type*>(last.mNode->mPrev);
               if (comp(*last, *mid))
               {
                  mid.mNode->mPrev->mNext = last.mNode;
                  last.mNode->mNext->mPrev = mid.mNode;

                  mid.mNode->mNext = last.mNode->mNext;
                  last.mNode->mPrev = mid.mNode->mPrev;

                  mid.mNode->mPrev = last.mNode;
                  last.mNode->mNext = mid.mNode;
